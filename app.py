...
# FastAPI REST API
rest_app = FastAPI(title="工地設備管理 API")


@rest_app.get("/api/equipment/{equipment_id}")
def api_get_equipment(equipment_id: str):
    equip = EQUIPMENT_DB.get(equipment_id)
    if not equip:
        return {"error": f"找不到設備 {equipment_id}"}
    return {"equipment_id": equipment_id, **equip}


@rest_app.get("/api/equipment")
def api_list_equipment(status: str | None = None):
    if status:
        return {eid: e for eid, e in EQUIPMENT_DB.items() if e["status"] == status}
    return EQUIPMENT_DB
